...
class GoldClass:
    ...
    def grouping(self, df):
        ...
        # We need to drop the null Customer because
        # we can´t track the correct ID if it´s null
        grouped_df = grouped_df[grouped_df["CustomerID"] != "0"]
        grouped_df.rename({"total_price_per_item": "total"},
                          axis=1, inplace=True)
    ...
    def creating_target(self, grouped_df):
        ...
        grouped_df["NextPurchaseDate"] = grouped_df.groupby("CustomerID")[
            "DateColumn"
        ].shift(-1)
        grouped_df["days_since_last_purchase"] = (
            grouped_df["NextPurchaseDate"] - grouped_df["DateColumn"]
        )
        grouped_df["days_since_last_purchase"] = grouped_df[
